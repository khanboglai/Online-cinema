...
class DataPreparer(StageABC):
    ...
    def _prepare_users(self, users_df: pd.DataFrame) -> pd.DataFrame:
        ...
        prepared_users["sex"] = prepared_users["sex"].map({"М": 1, "Ж": 0}).astype(np.int8)
    ...
    def _prepare_items(self, items_df: pd.DataFrame) -> pd.DataFrame:
        prepared_items = items_df[["item_id", "content_type", "title", "release_year", "genres", "countries", "for_kids", "age_rating", "rating_kp"]]
    ...
    def _prepare_interactions(self, interactions_df: pd.DataFrame) -> pd.DataFrame:
        ...
        prepared_interactions["datetime"] = pd.to_datetime(prepared_interactions["datetime"])

        prepared_interactions = prepared_interactions[prepared_interactions["total_dur"] > 300].reset_index(drop = True)
        prepared_interactions["weight"] = np.where(prepared_interactions['watched_pct'] > 50, np.int8(3), np.int8(1))
